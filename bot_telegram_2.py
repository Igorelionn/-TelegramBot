# -*- coding: utf-8 -*-
"""
Bot Telegram para envio de sinais em canais.
Versão 1.0 - Apenas Bot 1
"""
import requests
import schedule
import time
...
def send_message():
    ...
    try:
        ...
        if "NEAR (OTC)" in nome_ativo_exibicao or asset == "NEAR (OTC)":
            ...
            expiracao_texto = f"â³ Expiração: {tempo_expiracao_minutos} minutos ({expiracao_time.strftime('%H:%M')})"
        elif categoria == "Blitz":
            expiracao_segundos = random.choice([5, 10, 15, 30])
            tempo_expiracao_minutos = expiracao_segundos / 60
            expiracao_texto = f"â³ Expiração: {expiracao_segundos} segundos"
        elif categoria == "Digital":
            tempo_expiracao_minutos = random.choice([1, 3, 5])
            expiracao_time = entry_time + timedelta(minutes=tempo_expiracao_minutos)
            if tempo_expiracao_minutos == 1:
                expiracao_texto = f"â³ Expiração: 1 minuto ({expiracao_time.strftime('%H:%M')})"
            else:
                expiracao_texto = f"â³ Expiração: {tempo_expiracao_minutos} minutos ({expiracao_time.strftime('%H:%M')})"
            ...
            expiracao_texto = "â³ Expiração: até 5 minutos"
        ...
        for chat_id in CHAT_IDS:
            try:
                ...
                canal_message = (
                    f"âš ï¸TRADE RÁPIDOâš ï¸\n\n"
                    f"ðŸ'µ Ativo: {nome_ativo_exibicao}\n"
                    f"ðŸ·ï¸ Categoria: {categoria}\n"
                    f"{emoji} {action}\n"
                    f"âž¡ Entrada: {entry_time.strftime('%H:%M')}\n"
                    f"{expiracao_texto}\n"
